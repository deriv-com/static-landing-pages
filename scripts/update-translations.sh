...
NEW_TRANSLATION_BRANCH='landing_pages_translations'
...
if [[ $(git config --get remote.origin.url) =~ (binary-com|binary-static-deployed)/translations-static ]]; then
...
message "Updating landing-pages-static translations branch" &&
...
then
    ...
    cd $(git rev-parse --show-toplevel) && cd /scripts && node extract-translations.js &&
    message "Uploading source file to Crowdin"
    cd $(git rev-parse --show-toplevel) && source ~/.bash_profile && crowdin upload sources
    message "Complete, new translations have been uploaded to Crowdin"
fi

confirm "Download landing-pages-static files and update javascript texts?" &&
if [[ $REPLY =~ ^[Yy]$ ]]
then
    message "Downloading landing-pages-static files from Crowdin (*.json)" &&
...
then
    ...
    message "Committing"
    git commit -a -m "Update landing-pages-static translations" &&
    message "Pushing"
